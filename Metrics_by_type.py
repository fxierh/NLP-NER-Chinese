...
# with open("./output_human_labeled/test_golden_true_truncated.txt", "r", encoding='utf-8') as f:
with open("./output_pseudo_label/test_golden_true_truncated.txt", "r", encoding='utf-8') as f:
    lines = f.readlines()
    for index, line in enumerate(lines):
        if pattern.match(line) and lines[index+1] == '\n' and not line == "\n":
            token_types_golden_true.append([])
        if not (pattern.match(line) and lines[index+1] == '\n') and not line == "\n":
            token_types_golden_true[-1].append(line.strip().split(sep='\t')[-1])

# with open("./output_human_labeled/test_test_predictions.txt", "r", encoding='utf-8') as f:
with open("./output_pseudo_label/test_test_predictions.txt", "r", encoding='utf-8') as f:
    lines = f.readlines()
    for index, line in enumerate(lines):
        if pattern.match(line) and lines[index+1] == '\n' and not line == "\n":
...
# with open("./output_human_labeled/metrics_by_type_" + str(support) + ".json", "w", encoding='utf-8') as outfile:
with open("./output_pseudo_label/metrics_by_type_" + str(support) + ".json", "w", encoding='utf-8') as outfile:
